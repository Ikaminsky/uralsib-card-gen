...
export const CreditCard = ({ image, name }: { image: string, name: string }) => {
    return (
        <div className=''>
            <div id="card" className="relative w-96 h-60 rounded-2xl font-mono text-white overflow-hidden cursor-pointer transition-all duration-500" style={{
                transition: "0.6s;transform-style: preserve-3d"
            }}>
                <img src={image} style={{ zIndex: 40,
                    position: 'absolute',
                    mixBlendMode: 'screen' }}/>
                <div
                    className="absolute top-0 left-0 w-full h-full flex flex-col justify-center gap-6 p-6 bg-gradient-to-tr from-gray-900 to-gray-700 transition-all duration-100 delay-200 z-20" style={{transform: "rotateY(0deg)" }}>
